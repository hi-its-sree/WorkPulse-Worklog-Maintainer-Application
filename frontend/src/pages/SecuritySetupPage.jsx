...
import { fetchCurrentUser, getSecurityQuestions, setupSecurityQuestions } from '../lib/auth.js';
...
const SecuritySetupPage = () => {
  ...
  useEffect(() => {
    ...
    const loadQuestions = async () => {
      try {
        ...
        // The questions are already answered — the stored account details were just
        // out of date. Refresh them and carry on instead of stranding the user here.
        ...
        setError(err.response?.data?.message || strings.security.loading);
      }
    };
  ...
  const handleSubmit = async (event) => {
    ...
    if (questions.some((question) => !answers[question.key]?.trim())) {
      setError(strings.security.answerAll);
      return;
    }
    ...
    try {
      ...
      setStatusMessage(strings.security.saved);
      navigate('/dashboard');
    } catch (err) {
    ...
    <div className="flex min-h-screen items-center justify-center px-4 py-10">
      ...
      >
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-4 text-3xl font-semibold text-slate-900">{strings.security.title}</h1>
          <p className="mt-2 text-sm text-slate-500">
            {strings.security.subtitle}
          </p>
        </div>
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          {questions.length === 0 && !error && (
            <p className="text-sm text-slate-500">{strings.security.loading}</p>
          )}
          ...
          {questions.map((question) => (
            <div key={question.key}>
              ...
              <input
                ...
                placeholder={strings.security.answerPlaceholder}
                required
              />
            </div>
          ))}
          ...
          <button type="submit" className="app-action-btn w-full" disabled={saving || questions.length === 0}>
            {saving ? strings.security.saving : strings.security.saveButton}
          </button>
        </form>
      </motion.div>
    </div>
